...
class Wall:
    ...
    def move(self):
        if self.vert:
            ...
            self.x += self.speed
        if (
            (self.vert and (self.y < 5 or self.y > 350)) or
...
class Tank:
    ...
    def draw(self):
        if time.time() > self.flash_time_end or time.time()%0.1 < 0.05:
            ...
            screen.blit(rotated,
                        (self.x+self.img.get_width()/2-rotated.get_width()/2,
    ...
    def hit_wall(self,wall):
            return ((wall.vert and 
                     pygame.Rect((wall.x,wall.y),vert_wall_image.get_size()).colliderect((self.x,self.y+10),(60,60))) or
                    (not wall.vert and pygame.Rect((wall.x,wall.y),wall_image.get_size()).colliderect((self.x,self.y+10,60,60))))
"==========================================================================================================="
class Shell:
